...
const ITEM_TYPES = new Set(["widget", "alert", "text", "image", "icon", "shape", "group"]);
...
function normalizeItemProps(type, props) {
  ...
  switch (type) {
    ...
    case "image":
      return {
        src: normalizeImageUrl(source.src),
...
// client pourrait coller une image encodee en base64 dans le prompt "URL"
// et faire gonfler overlay.json (un fichier JSON pense pour rester lisible
// a la main) alors que la consigne produit est "URL uniquement, pas d'upload".
...
async function createOverlay({ name, description, icon, id, canvas }) {
  const now = Date.now();
