...
// Chaque projet a son propre overlays/ (library/<projectId>/overlays) : toutes
// les fonctions ci-dessous prennent donc projectId en premier parametre.
function overlaysRootFor(projectId) {
  return join(LIBRARY_ROOT, projectId, "overlays");
}
...
async function listOverlays(projectId) {
  const overlaysRoot = overlaysRootFor(projectId);
  if (!existsSync(overlaysRoot)) return [];
  const overlays = [];
  const directories = await readdir(overlaysRoot, { withFileTypes: true });

  for (const directory of directories) {
    if (!directory.isDirectory()) continue;
    const overlayDirectory = join(overlaysRoot, directory.name);
...
async function getOverlayInfo(projectId, overlayId) {
  if (typeof overlayId !== "string" || !/^[a-z0-9][a-z0-9-]*$/.test(overlayId)) return null;
  const directory = join(overlaysRootFor(projectId), overlayId);
...
async function createOverlay(projectId, { name, description, icon, id, canvas, sourcePlatform, sourceOverlayId }) {
  const now = Date.now();
  ...
  const directory = join(overlaysRootFor(projectId), id);
  await mkdir(directory, { recursive: true });
  await writeFile(join(directory, "overlay.json"), `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
  return overlayFromManifest(id, manifest);
}

async function updateOverlayMetadata(projectId, overlayId, { name, description, icon, canvas }) {
  const overlayInfo = await getOverlayInfo(projectId, overlayId);
...
// Associe (ou dissocie, si sourcePlatform/sourceOverlayId valent null) un
// overlay local deja existant a un overlay distant, sans toucher a ses
// items/canvas - contrairement a l'import, qui lui ecrase la mise en page.
// Utile pour un overlay construit a la main dans le Lab qui correspond a un
// overlay deja present sur StreamElements : une fois lie, un futur import de
// ce meme overlay distant le mettra a jour au lieu d'en creer un doublon
// (meme logique de deduplication que l'import, cf. server.mjs).
async function updateOverlaySource(projectId, overlayId, { sourcePlatform, sourceOverlayId }) {
  const overlayInfo = await getOverlayInfo(projectId, overlayId);
  if (!overlayInfo) return null;
  const manifestPath = join(overlayInfo.directory, "overlay.json");
  const manifest = await readJson(manifestPath);
  const updated = {
    ...manifest,
    sourcePlatform: SOURCE_PLATFORMS.has(sourcePlatform) ? sourcePlatform : null,
    sourceOverlayId: typeof sourceOverlayId === "string" && sourceOverlayId ? sourceOverlayId.slice(0, 200) : null,
    updatedAt: Date.now()
  };
  await writeFile(manifestPath, `${JSON.stringify(updated, null, 2)}\n`, "utf8");
  return overlayFromManifest(overlayId, updated);
...
async function replaceOverlayItems(projectId, overlayId, items) {
  const overlayInfo = await getOverlayInfo(projectId, overlayId);
  if (!overlayInfo) return null;
  const manifestPath = join(overlayInfo.directory, "overlay.json");
  const manifest = await readJson(manifestPath);
...
async function replaceOverlayGuides(projectId, overlayId, guides) {
  const overlayInfo = await getOverlayInfo(projectId, overlayId);
  if (!overlayInfo) return null;
  const manifestPath = join(overlayInfo.directory, "overlay.json");
  const manifest = await readJson(manifestPath);
...
async function deleteOverlay(projectId, overlayId) {
  const overlayInfo = await getOverlayInfo(projectId, overlayId);
...
async function duplicateOverlay(projectId, overlayId, newId, newName) {
  const overlayInfo = await getOverlayInfo(projectId, overlayId);
  if (!overlayInfo) return null;
  ...
  const directory = join(overlaysRootFor(projectId), newId);
  await mkdir(directory, { recursive: true });
