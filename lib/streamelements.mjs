...
const PLACEHOLDER_SOURCE_TYPES = { video: "video", "se-widget-group": "group", "se-widget-alert-box": "alert-box" };
...
/**
 * Convertit une entree brute de overlay.widgets (API StreamElements) en soit
 * un Custom Widget importable (html/css/js/fields reels), soit un repere non
 * editable (widget natif StreamElements : Chat Box, Alert Box, Event List non
 * personnalise, groupe SE, video...).
 *
 * Le champ `type` interne de StreamElements (ex. "se-widget-custom-event-list")
 * N'EST PAS un discriminant fiable : un Event List jamais personnalise porte
 * le meme `type` qu'un vrai Custom Widget construit dessus. Verifie contre un
 * vrai compte (6 overlays / 26 widgets) : le seul critere fiable est la
 * presence reelle de code dans variables.html/css/js.
 */
export function convertOverlayWidget(rawWidget) {
  const w = rawWidget || {};
  const variables = w.variables || {};
  const css = w.css || {};
  const geometry = {
    name: typeof w.name === "string" && w.name.trim() ? w.name.trim() : "Widget StreamElements",
    x: parseCssPixelValue(css.left, 0),
    y: parseCssPixelValue(css.top, 0),
    w: parseCssPixelValue(css.width, 320),
    h: parseCssPixelValue(css.height, 180),
    z: parseCssPixelValue(css["z-index"], 1)
  };

  const hasCode = ["html", "css", "js"].every((key) => typeof variables[key] === "string" && variables[key].trim().length > 0);
  if (hasCode) {
    let fields = {};
    if (typeof variables.fields === "string" && variables.fields.trim()) {
      try {
        const parsed = JSON.parse(variables.fields);
        if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) fields = parsed;
      } catch {
        // Champ fields illisible : on importe quand meme le widget, sans reglages.
      }
    }
    return { kind: "custom", ...geometry, html: variables.html, css: variables.css, js: variables.js, fields };
  }

  return { kind: "placeholder", ...geometry, sourceType: PLACEHOLDER_SOURCE_TYPES[w.type] || "native" };
}
