...
function buildStore(db) {
  const stmts = {
    ...
    insertContactMessage: db.prepare(`
      INSERT INTO contact_messages (id, user_id, first_name, last_name, email, subject, message, created_at)
      VALUES (@id, @userId, @firstName, @lastName, @email, @subject, @message, @now)
    `),
  ...
  return {
    ...
    insertContactMessage({ userId, firstName, lastName, email, subject, message }) {
      const id = randomUUID();
      const now = Date.now();
      stmts.insertContactMessage.run({ id, userId: userId ?? null, firstName, lastName, email, subject, message, now });
