...
test("saveContactMessage enregistre un message sans utilisateur connecté", () => {
  ...
  const saved = saveContactMessage({
    userId: null, firstName: "Ada", lastName: "Lovelace",
...
test("validateContactMessage rejette un email invalide", () => {
  assert.throws(() => validateContactMessage({
    firstName: "Ada", lastName: "Lovelace", email: "pas-un-email", subject: "x", message: "x"
  }));
});

test("validateContactMessage rejette un champ vide", () => {
  assert.throws(() => validateContactMessage({
    firstName: "", lastName: "Lovelace", email: "a@b.com", subject: "x", message: "x"
  }));
});
