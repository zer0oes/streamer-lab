...
function dispatchChatMessage({ name, message, color = "#9f75ff", badges }) {
  let messageBadges = badges;
  if (!messageBadges) {
    const demoBadge = demoTwitchBadges[nextDemoBadge++ % demoTwitchBadges.length];
    messageBadges = demoBadge ? [demoBadge] : [];
  }
...
function sendPresetEvent() {
  ...
  const name = elements.eventName.value.trim() || "Anonyme";
  const amount = Number(elements.eventAmount.value) || 0;
  const message = elements.eventMessage.value;
  let event;
