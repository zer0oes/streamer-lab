...
function applyRoleTheme(element, role) {
  ...
  const opacity = clampOpacity(fields.backgroundOpacity, 0.7);

  element.style.setProperty("--role-start", start);
  element.style.setProperty("--role-end", end);
  element.style.setProperty("--role-start-soft", mixWithDark(start, 0.38, opacity));
  element.style.setProperty("--role-end-soft", mixWithDark(end, 0.28, opacity));
  element.style.setProperty("--role-border", toRgba(end, 0.76));
  element.style.setProperty("--role-start-glow", toRgba(start, 0.24));
  element.style.setProperty("--role-end-glow", toRgba(end, 0.2));
}
...
function applyGlobalTheme() {
  ...
  root.style.setProperty("--bubble-border-alpha", toRgba(fields.bubbleBorder || "#34385F", 0.42));
}
